...
test('new-session route reuses the remembered welcome agent before falling back to the first agent', () => {
    // 无 query 时先复用激活网关条目里记住的选择（用户上次在欢迎页
    // 下拉里的明确选择），校验存在性（已删除则跳过），最后才回落第一个 agent。
    // 必须按条目读：各服务器 agent id 互不相通，全局单值会在切换账号时窜台
    assert.match(
        source,
        /const rememberedAgent = settingsStore\.activeGateway\?\.lastNewSessionAgentId \?\? ''/,
        'new-session route should read the remembered welcome agent from the active gateway entry',
    )
...
test('welcome dropdown selection persists to the settings store', () => {
    // selectWelcomeAgent（用户明确选择）必须写回 settingsStore 的激活条目：
    // 否则 /new 与冷启动兜底拿不到记住的值，或各服务器之间互相覆盖
    ...
    assert.match(
        ...
        'welcome dropdown selection should be persisted into the active gateway entry',
    )
})
