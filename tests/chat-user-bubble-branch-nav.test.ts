...
// 但仅限「分支尾部没有已渲染 assistant 回复」的场景（isLastMessage 门控）：
...
test('user toolbar renders branch navigation for multi-branch sessions', () => {
    ...
    assert.ok(
        section.includes('isLastMessage && branchInfo && branchInfo.siblings.length > 1'),
...
test('user branch navigation sits in normal mode after delete button', () => {
    ...
    const navIndex = section.indexOf('isLastMessage && branchInfo', normalModeStart)
