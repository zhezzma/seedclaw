...
import { useCronState, type TaskJob, type CronRunLogEntry, type CronFormState } from '../composables/useCronState'
...
const getScheduleDisplay = (job: TaskJob) => {
    ...
    if (job.scheduleKind === 'every') return t('cron.display.every', { amount: job.everyAmount, unit: t(`cron.units.${job.everyUnit}`) })
...
const handleSave = async () => {
    ...
    if (!form.value.agentId) return modalError.value = "Agent is required"
    if (form.value.scheduleKind === 'cron' && !form.value.cronExpr) return modalError.value = "Cron Expression is required"
    if (form.value.scheduleKind === 'every' && (!form.value.everyAmount || !form.value.everyUnit)) return modalError.value = "Interval details required"
    if (form.value.scheduleKind === 'at' && !form.value.scheduleAt) return modalError.value = "Schedule time is required"
...
<template>
    <div class="flex flex-col h-full bg-base-200">
            ...
            <div class="modal-box w-full max-w-lg">
                ...
                <div class="space-y-4 max-h-[60vh] overflow-y-auto px-1">
                    ...
                    <div class="form-control w-full">
                        <label class="label"><span class="label-text">{{ $t('cron.form.name') }} <span
                    ...
                    <!-- Agent -->
                    <div class="form-control w-full">
                        <label class="label"><span class="label-text">{{ $t('cron.form.agentId') }} <span
                                    class="text-error">*</span></span></label>
                    ...
                    <div class="form-control">
                        ...
                        <div class="join w-full">
                            <input class="join-item btn flex-1" type="radio" name="options"
                    ...
                    <div v-if="form.scheduleKind === 'every'" class="flex gap-2">
                        <div class="form-control flex-1">
                            <label class="label"><span class="label-text">{{ $t('cron.form.everyAmount') }} <span
                                        class="text-error">*</span></span></label>
                            <input v-model="form.everyAmount" type="number" class="input input-bordered w-full" />
                        </div>
                        <div class="form-control flex-1">
                            <label class="label"><span class="label-text">{{ $t('cron.form.everyUnit') }} <span
                                        class="text-error">*</span></span></label>
                    ...
                    <div v-if="form.scheduleKind === 'cron'" class="form-control w-full">
                        <label class="label"><span class="label-text">{{ $t('cron.form.cron') }} <span
                                    class="text-error">*</span></span></label>
                    ...
                    <div v-if="form.scheduleKind === 'at'" class="form-control w-full">
                        <label class="label"><span class="label-text">{{ $t('cron.form.scheduleAt') }} <span
                                    class="text-error">*</span></span></label>
        ...
        <!-- Logs Modal (Unchanged largely, just checking props) -->
                            ...
                            <tr v-for="(log, idx) in cronRunLogs" :key="idx">
                                ...
                                <td>
                                    <span class="badge badge-xs"
                                        :class="{ 'badge-success': log.status === 'OK' || log.status === 'ok', 'badge-error': log.status === 'error', 'badge-warning': log.status === 'skipped' }">{{
                                            log.status }}</span>
                                </td>
