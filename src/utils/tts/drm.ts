...
// SHA-256 implementation with proper unsigned 32-bit arithmetic
function sha256(ascii: string) {
    ...
    const data = new Uint8Array(Math.ceil((ascii.length + 1 + 8) / 64) * 64);
    for (let i = 0; i < ascii.length; i++) data[i] = ascii.charCodeAt(i);
    data[ascii.length] = 0x80;

    const dataView = new DataView(data.buffer);
    dataView.setUint32(data.length - 4, len, false);
